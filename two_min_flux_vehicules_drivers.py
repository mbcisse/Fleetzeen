    ...
    for var in alpha_taxis_ids:
        ...
        if db_last_update_alpha is not None and db_last_update_alpha !='false':
            ...
            if odoo_last_update > db_last_update_alpha:
                #print("db_last_update: ",   db_last_update_alpha)
                #print("odoo_last_update: ", odoo_last_update)
                result.append(var)
            else:
                pass
                #print("No need to update the data base")
        else:
            pass
            #print("This id not found in the database: ", var["id"])
    return result
...
def get_fleet_vehicule_assignation(driver_ids):
    ...
    if count==1:
        ...
        if db_last_update != 'false' and odoo_last_update  != 'false':
            db_last_update  = datetime.strptime(db_last_update, "%Y-%m-%d %H:%M:%S")
            odoo_last_update= datetime.strptime(odoo_last_update, "%Y-%m-%d %H:%M:%S")
            print("db_last_update get_fleet_vehicule_assignation: ",   db_last_update)
            print("odoo_last_update get_fleet_vehicule_assignation : ", odoo_last_update)
        ...
        elif db_last_update =='false' and odoo_last_update !='false':
            return  id_vehicule_prec[0], id_vehicule_actual[0], start_date_vehic_prec, start_date_vehic_actual, end_date_vehic_prec, end_date_vehic_actual, last_update, last_update
        ...
        if veh_to_take=='actuel':        
            if db_last_update != 'false' and odoo_last_update  != 'false':
                ...
                print("odoo_last_update get_fleet_vehicule_assignation: ", odoo_last_update)
                if  odoo_last_update > db_last_update:
                    print("Yes odoo_last_update get_fleet_vehicule_assignation: ", odoo_last_update)
                ...
                return  id_vehicule_prec[0], id_vehicule_actual[0], start_date_vehic_prec, start_date_vehic_actual, end_date_vehic_prec, end_date_vehic_actual, last_update_prec, last_update_actual
...
def two_min_getting_drivers_vehicules_datas():
    ...
    for elem in drivers_ids:
        #print("Elem: ", elem[0])
        drivers_ids_alpha=get_alpha_taxis_with_ids(elem[0])
        drivers_ids_res_partner= get_res_partner_with_ids(elem[0])
        drivers_fleet_vehicule = get_fleet_vehicule_assignation(elem[0])     
        
        if len(drivers_ids_alpha) > 0:
            for datas_alpha_taxis in drivers_ids_alpha:
